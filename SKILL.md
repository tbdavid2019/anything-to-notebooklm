...
代理應依照「二、支援的輸入來源」判斷目前輸入屬於哪一類，避免在不同章節重複定義來源類型。

解析時只需做三件事：

1. 判斷是單一來源還是多來源
2. 判斷是遠端來源、本機來源、搜尋查詢或使用者直接貼上的文字
3. 為每個來源指派統一的 `source_type`，供後續正規化與命名使用
...
若是 `x.com` / `twitter.com` 來源，代理應優先補充：
